...
export const ACHIEVEMENTS: readonly AchievementDefinition[] = [
  ...
  },
  {
    ...
    condition: (_lifetime, snapshot) =>
      ...
      Object.values(snapshot.tasks.byDayOfWeek).filter(
  ...
  },
  {
    ...
    description:
      "No single blocked app accounts for most of your recent attempts.",
  ...
  },
  {
    ...
    title: "Iron session",
    description:
      "Completed a focus session without opening a blocked app once.",
    icon: "flash-outline",
  ...
  },
  {
    ...
    hidden: true,
    condition: (_lifetime, snapshot) =>
      snapshot.tasks.estimationErrorMinutes.filter((error) => error <= -30)
        .length >= 3,
  },
  {
    id: "BACK_AGAIN",
...
export function evaluateAchievements(
  ...
  return ACHIEVEMENTS.filter((achievement) =>
    achievement.condition(lifetime, snapshot),
  );
}
...
export async function syncAchievements(
  ...
  return {
    definitions: ACHIEVEMENTS.filter((achievement) =>
      nextEarnedIds.includes(achievement.id),
    ),
